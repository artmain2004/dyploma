...
const LoginPage = () => {
  ...
  useEffect(() => {
    ...
    const rememberedFlag = localStorage.getItem("rememberMe") === "true";
    if (rememberedEmail) {
  ...
  return (
    <div className="mx-auto max-w-md rounded-2xl bg-white p-8 shadow-card">
      ...
      <p className="mt-4 text-sm text-slate-500">
        <Link to="/forgot-password" className="font-semibold text-slate-700 hover:text-slate-900">Nie pamietasz hasla?</Link>
      </p>
      <p className="mt-6 text-sm text-slate-500">
        Nie masz konta? <Link to="/register" className="font-semibold text-slate-700 hover:text-slate-900">Utworz</Link>
      </p>
    </div>
  );
};
